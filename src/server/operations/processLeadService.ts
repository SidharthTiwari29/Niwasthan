...
import { generateFounderReportMetrics, type ReportPeriod } from "./founderReportService";
...
export async function correlateProcessLead(period: ReportPeriod): Promise<ProcessLeadCorrelation> {
  const [observations, openEscalations, openIncidents, operationalEvents] = await Promise.all([
    ...
    prisma.agentIncident.count({ where: { status: { notIn: ["RESOLVED", "VERIFIED"] } } }),
    prisma.operationalEvent.count({ where: { occurredAt: { gte: period.start, lt: period.end } } }),
  ]);
  ...
  return {
    ...
    specialistSignals: observations.map((observation) => ({
      ...
      actionCount: observation.decisions.reduce((sum, decision) => sum + decision.actions.length, 0),
    })),
    openEscalations,
    openIncidents,
    operationalEvents,
