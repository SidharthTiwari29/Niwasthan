...
export default async function PropertyDetailPage({
  ...
  return (
    <div className="space-y-10">
      ...
      <section className="relative overflow-hidden rounded-[1.75rem] bg-ink px-6 py-8 text-paper shadow-xl shadow-ink/10 md:px-10 md:py-10">
        ...
        <div className="relative max-w-4xl">
          <div className="flex flex-wrap items-center gap-2 font-mono text-[9px] uppercase tracking-[0.22em] text-paper/55">
            ...
            <span>{t("propertyIdLabel", { id: property.id })}</span>
          </div>
          <h1 className="mt-5 font-display text-[clamp(3rem,7vw,6rem)] font-semibold leading-[0.84] tracking-[-0.06em]">
            {property.name}
          </h1>
          <div className="mt-5 flex flex-wrap gap-x-5 gap-y-2 font-body text-sm text-paper/65">
          ...
          <div className="mt-8 flex flex-wrap gap-3">
            <Link
              href={`/properties/${propertyId}/intelligence`}
              className="inline-flex items-center gap-2 rounded-full bg-brass px-5 py-3 font-body text-sm font-semibold text-ink"
            >
              Open intelligence ↗
            </Link>
            <Link
              href={`/properties/${propertyId}/floor-plan`}
              className="inline-flex items-center gap-2 rounded-full border border-paper/20 px-5 py-3 font-body text-sm text-paper/80 transition-colors hover:border-brass"
            >
              {t("uploadFloorPlan")} →
            </Link>
            <Link
      ...
      <section className="grid gap-4 md:grid-cols-3">
        <div className="rounded-2xl border border-ink/10 bg-white p-5">
          ...
          <p className="mt-3 font-display text-3xl font-semibold">
            {rooms.length > 0 ? "In progress" : "Not started"}
          </p>
          <p className="mt-2 font-body text-xs leading-relaxed text-ink-soft">
            {rooms.length > 0
              ? `${rooms.length} room${rooms.length === 1 ? "" : "s"} represented in the home model.`
              : "Add a plan or room to begin the spatial model."}
          </p>
        </div>
        <div className="rounded-2xl border border-ink/10 bg-white p-5">
          <p className="font-mono text-[9px] uppercase tracking-[0.18em] text-ink-soft">
          ...
          <p className="mt-3 font-display text-3xl font-semibold">
            {targetBudgetRupees !== null
              ? `₹${targetBudgetRupees.toLocaleString("en-IN")}`
              : "Unknown"}
          </p>
          <p className="mt-2 font-body text-xs leading-relaxed text-ink-soft">
            Target only until quantities, products, and evidence are confirmed.
          </p>
        </div>
        <div className="rounded-2xl border border-ink/10 bg-white p-5">
          <p className="font-mono text-[9px] uppercase tracking-[0.18em] text-ink-soft">
          ...
          <p className="mt-2 font-body text-xs leading-relaxed text-ink-soft">
            A direction becomes meaningful when its downstream impact is
            visible.
          </p>
        </div>
      </section>
      <section className="flex flex-wrap items-center justify-between gap-4 rounded-[1.5rem] border border-ink/10 bg-white px-6 py-5">
        <div>
          <p className="font-mono text-[9px] uppercase tracking-[0.18em] text-laterite">
            After the decision
          </p>
          <p className="mt-2 font-display text-2xl font-semibold">
            Build & handover stays visible.
          </p>
          <p className="mt-1 font-body text-sm text-ink-soft">
            Quotes, orders, delivery, installation, snags, and handover appear
            from real records.
          </p>
        </div>
        <div className="flex flex-wrap gap-2">
          <Link
            href={`/properties/${propertyId}/intelligence`}
            className="rounded-full border border-ink/15 px-4 py-3 font-body text-sm font-semibold text-ink"
          >
            What-if & Smart Home
          </Link>
          <Link
      ...
      <section className="rounded-[1.5rem] border border-ink/10 bg-[#e9e1d5] p-6 md:p-8">
        <div className="grid gap-6 md:grid-cols-[1fr_auto] md:items-end">
          <div>
            <p className="font-mono text-[9px] uppercase tracking-[0.22em] text-laterite">
              Next best step
            </p>
            <h2 className="mt-3 font-display text-4xl font-semibold leading-[0.92] tracking-[-0.045em]">
              Make the real home more legible.
            </h2>
            <p className="mt-4 max-w-xl font-body text-sm leading-relaxed text-ink-soft">
              Confirm the unknowns before choosing a look. Niwasthan will carry
              this context into design, catalogue, budget, and buildability.
            </p>
          </div>
          <Link
            href={
              rooms.length > 0
                ? `/properties/${propertyId}/rooms/${rooms[0]?.id}/understanding`
                : `/properties/${propertyId}/floor-plan`
            }
            className="inline-flex items-center justify-center rounded-full bg-ink px-5 py-3 font-body text-sm font-semibold text-paper"
          >
            {rooms.length > 0 ? t("confirmDimensions") : t("uploadFloorPlan")} →
          </Link>
        </div>
      </section>
      <div className="mt-10">
