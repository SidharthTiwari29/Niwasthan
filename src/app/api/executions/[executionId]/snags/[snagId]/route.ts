...
const paramsSchema = z.object({ executionId: z.string().cuid(), snagId: z.string().cuid() });
const bodySchema = z.object({ status: z.enum(["OPEN", "IN_REVIEW", "RESOLVED", "ACCEPTED"]) });
...
export const PATCH = withErrorHandling(async (request: Request, { params }: RouteParams) => {
  ...
  return NextResponse.json({ snag: await executionEvidenceService.updateSnag(snagId, userId, status) });
});
