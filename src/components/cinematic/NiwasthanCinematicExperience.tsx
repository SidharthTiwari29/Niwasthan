...
const hotspots: Hotspot[] = [
  {
    ...
    brand: "Catalogue-selected brand",
    spec: "Large-format premium natural stone finish",
    warranty: "Catalogue verified",
    unitPrice: 0,
  },
  {
    id: "fluted",
    ...
    brand: "Catalogue-selected brand",
    spec: "Architectural fluted panel system",
    warranty: "Catalogue verified",
    unitPrice: 0,
  },
  {
    ...
    brand: "Catalogue-selected brand",
    spec: "Ambient + accent + task lighting",
    warranty: "Catalogue verified",
    unitPrice: 0,
  },
  {
    ...
    brand: "Catalogue-selected brand",
    spec: "Engineered quartz work surface",
    warranty: "Catalogue verified",
    unitPrice: 0,
  },
  {
    ...
    x: 77,
    y: 37,
    title: "Acrylic Cabinets",
    brand: "Catalogue-selected brand",
    spec: "Premium acrylic cabinet finish",
    warranty: "Catalogue verified",
    unitPrice: 0,
  },
  {
    ...
    brand: "Catalogue-selected brand",
    spec: "Integrated appliance package",
    warranty: "Catalogue verified",
    unitPrice: 0,
  },
  {
    ...
    x: 76,
    y: 46,
    title: "Premium Hardwood Wardrobe",
    brand: "Catalogue-selected brand",
    spec: "Full-height premium wardrobe system",
    warranty: "Catalogue verified",
    unitPrice: 0,
  },
  {
    ...
    brand: "Catalogue-selected brand",
    spec: "Acoustic-performance glazing",
    warranty: "Catalogue verified",
    unitPrice: 0,
  },
  {
    ...
    brand: "Catalogue-selected brand",
    spec: "Integrated desk + storage system",
    warranty: "Catalogue verified",
    unitPrice: 0,
  },
  {
    ...
    x: 63,
    y: 48,
    title: "Balcony Greenery",
    brand: "Catalogue-selected brand",
    spec: "Low-maintenance outdoor planting system",
    warranty: "Catalogue verified",
    unitPrice: 0,
  },
];

const roomTotals = ["Living", "Kitchen", "Master", "Kids", "Balcony"].map(
  (room) => ({ room, value: 0 }),
);
  ...
  return (
    <main ref={ref} className={styles.shell}>
      <section
        ...
        id="cinema"
        aria-label="Niwasthan Home Tour"
      >
        <div className={styles.stage}>
          <div className={styles.world}>
            <header className={styles.header} aria-label="Primary Navigation">
              <button className={styles.logo} onClick={() => jump(0)}>
                NIWASTHAN
              </button>
              <nav
              ...
              >
                {[0, 800, 2500, 3800, 4800, 5600].map((at, i) => (
                  <button
                    key={at}
                    onClick={() => jump(at)}
                  >{`0${i + 1} ${["Enter", "Discover", "Understand", "Price", "Design", "Build"][i]}`}</button>
                ))}
              </nav>
              <button
            ...
            <div className={styles.backStack}>
              <div
                className={styles.sceneLayer + " " + styles.layerEntrance}
                style={{
                  opacity: 1 - enter * 0.75,
                  transform: `scale(${1 + enter * 0.85})`,
                }}
              >
              ...
              {scenes.map((scene, i) => {
                ...
                const opacity =
                  discover *
                  (i === activeScene ? 1 : 0.04) *
                  (i === 4 ? 1 : fadeOut + fadeIn);
                const zoom =
                  1 + clamp((p - (800 + i * 300)) / 1000) * (0.2 + i * 0.012);
                return (
                  <div
                    key={scene.id}
                    className={`${styles.sceneLayer} ${styles[`layer${i}`]}`}
                    style={{
                      opacity,
                      transform: `scale(${zoom}) translate3d(calc(var(--mx) * ${-1.4 - i * 0.45}px), calc(var(--my) * ${-0.8 - i * 0.25}px), 0)`,
                    }}
                  >
            ...
            <section
              className={`${styles.storyPanel} ${styles.panelEnter}`}
              style={{ opacity: 1 - enter }}
            >
              <span className={styles.eyebrow}>01 · NIWASTHAN</span>
              <h1>ENTER YOUR HOME</h1>
              <p>Decide Smart. Live Better.</p>
              <span className={styles.scrollCue}>Scroll to enter ↓</span>
            </section>
            <section
              className={`${styles.storyPanel} ${styles.panelDiscover}`}
              style={{ opacity: discover * (1 - understand) }}
            >
              <span className={styles.eyebrow}>02 · DISCOVER</span>
              <h2>DISCOVER YOUR HOME</h2>
              <div className={styles.roomIndicator}>
                Living Room • Dining • Kitchen • Master • Kids • Balcony
              </div>
            </section>
            ...
            >
              {hotspots.map((h) => (
                <button
                  key={h.id}
                  className={`${styles.hotspot} ${h.scene === activeScene ? styles.hotspotActive : ""}`}
                  style={{
                    left: `${h.x}%`,
                    top: `${h.y}%`,
                    transform: `translate(-50%,-50%) scale(${h.scene === activeScene ? 1 : 0.35})`,
                  }}
                  onClick={() =>
                    setActiveHotspot(selected?.id === h.id ? null : h.id)
                  }
              ...
              {selected && (
                <aside className={styles.materialCard}>
                  ...
                  <dl>
                    ...
                    <dd>{selected.spec}</dd>
                    <dt>Warranty</dt>
                    <dd>{selected.warranty}</dd>
                    <dt>Unit Price</dt>
                    <dd>
                      {selected.unitPrice
                        ? fmt(selected.unitPrice)
                        : "Catalogue verified"}
                    </dd>
                  </dl>
                </aside>
            ...
            <section
              className={styles.storyPanel + " " + styles.panelUnderstand}
              style={{ opacity: understand * (1 - price) }}
            >
              <span className={styles.eyebrow}>03 · UNDERSTAND</span>
              <h2>UNDERSTAND YOUR HOME</h2>
              <p>
                Tap the gold points to inspect materials, brands,
                specifications, warranty and price intelligence.
              </p>
            </section>

            <section
              className={styles.boqBreakdownCard}
              style={{
                opacity: price,
            ...
            >
              <div>
                <span className={styles.eyebrow}>04 · PRICE YOUR HOME</span>
                <h2>PRICE YOUR HOME</h2>
              </div>
              <div className={styles.boqCard}>
                ...
                {roomTotals.map((r) => (
                  <div className={styles.boqRow} key={r.room}>
                    <span>{r.room}</span>
                    <b>{fmt(r.value)}</b>
                  </div>
                ))}
                <footer>Room-level BOQ • Product-level transparency</footer>
              </div>
            </section>
            ...
            >
              <span className={styles.eyebrow}>05 · DESIGN YOUR HOME</span>
              <h2>DESIGN YOUR HOME</h2>
              <div className={styles.processGrid}>
                <article>
                  <b>01</b>
                  <h3>Niwasthan Process</h3>
                  <p>Discovery → Design → Visualization → Handover</p>
                </article>
                <article>
                  <b>02</b>
                  <h3>AI + Design Intelligence</h3>
                  <p>Spatial optimization and material intelligence.</p>
                </article>
                <article>
                  <b>03</b>
                  <h3>Real-Time 3D Visualization</h3>
                  <p>Experience the home before you build it.</p>
                </article>
              </div>
            </section>
            ...
            <section
              ...
              style={{
                opacity: build,
                transform: `translateY(${40 - build * 40}px)`,
              }}
            >
              <span className={styles.eyebrow}>06 · BUILD YOUR HOME</span>
      ...
      <footer className={styles.footer}>
        <div className={styles.footerContent}>
          <div className={styles.footerBrand}>NIWASTHAN</div>
          ...
          <div className={styles.copyright}>
            © 2026 Niwasthan. All rights reserved.
          </div>
        </div>
      </footer>
    </main>
